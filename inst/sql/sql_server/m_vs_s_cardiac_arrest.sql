...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4169095,45766074,4171683,317302)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4169095,45766074,4171683,317302)
  and c.invalid_reason is null
...
WHERE C.condition_type_concept_id  in (44786629,38000183,38000199,44786627)
AND (YEAR(C.condition_start_date) - P.year_of_birth >= 18 and YEAR(C.condition_start_date) - P.year_of_birth <= 100)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-1,P.START_DATE) AND A.START_DATE <= DATEADD(day,1,P.START_DATE) AND A.visit_occurrence_id = P.visit_occurrence_id
